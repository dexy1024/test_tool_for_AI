  ...
  <div class="price-page">
    ...
    <el-card v-if="executionResults.length > 0" class="result-card" shadow="hover">
      ...
      >
        ...
        <el-table-column prop="sku名称" label="SKU名称" min-width="150" show-overflow-tooltip />
        
        <el-table-column prop="识别价格" label="识别价格" width="120" align="right">
          <template #default="{ row }">
            <span v-if="row['识别价格']">{{ formatPrice(row['识别价格']) }}</span>
            <span v-else>无</span>
          </template>
        </el-table-column>
        
        <el-table-column prop="sku价格区间" label="SKU价格区间" width="180">
          <template #default="{ row }">
            <span v-if="row['sku价格区间']">{{ row['sku价格区间'] }}</span>
            <span v-else>无</span>
          </template>
        </el-table-column>
        
        <el-table-column prop="是否合格" label="是否合格" width="120" align="center">
          <template #default="{ row }">
            <el-tag :type="row['是否合格'] === '合格' ? 'success' : 'danger'" size="small">
              {{ row['是否合格'] || '无' }}
            </el-tag>
          </template>
        </el-table-column>
...
export default {
  ...
  setup() {
    ...
    const generatePriceRange = (price) => {
      ...
      if (isNaN(numPrice)) return '无'
      
      // 生成价格区间（±10%）
      const min = (numPrice * 0.9).toFixed(2)
      const max = (numPrice * 1.1).toFixed(2)
      return `¥${min} - ¥${max}`
    }

    // 模拟价格识别
    const simulatePriceRecognition = async (row, index) => {
      // 模拟识别延迟
      await new Promise(resolve => setTimeout(resolve, 500 + Math.random() * 1000))
      
      // 生成模拟数据
      const recognizedPrice = Math.random() * 100 + 10 // 10-110之间的随机价格
      const skuNumber = String(Math.floor(Math.random() * 10000)).padStart(5, '0')
      const skuName = `SKU-${skuNumber}`
      ...
      return {
        ...row,
        'sku名称': skuName,
        '识别价格': recognizedPrice,
        'sku价格区间': generatePriceRange(recognizedPrice),
        '是否合格': isQualified ? '合格' : '不合格',
        '不合格原因': isQualified ? '' : '识别价格超出允许范围',
        '人工判定': '' // 初始化为空，等待用户判定
      }
    }
    ...
    const exportToExcel = () => {
      ...
      try {
        ...
        const colWidths = headers.map(header => {
          ...
          if (header === 'sku名称') return { wch: 20 }
          if (header === '识别价格') return { wch: 15 }
          if (header === 'sku价格区间') return { wch: 20 }
          if (header === '是否合格') return { wch: 12 }
          if (header === '不合格原因') return { wch: 25 }
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, '价格识别结果')

        // 生成文件名（包含时间戳）
        const now = new Date()
        const timestamp = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}${String(now.getSeconds()).padStart(2, '0')}`
        const fileName = `价格识别结果_${timestamp}.xlsx`
    ...
    const startExecute = async () => {
      ...
      try {
        ...
        for (let i = 0; i < expandedData.length; i++) {
          ...
          const result = await simulatePriceRecognition(row, i)
        ...
        const successCount = executionResults.value.filter(r => r['是否合格'] === '合格').length
        const failCount = executionResults.value.length - successCount

        ElMessage.success(
          `执行完成！合格：${successCount} 条，不合格：${failCount} 条`
        )
      } catch (error) {
